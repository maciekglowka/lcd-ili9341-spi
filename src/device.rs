    ...
    W: SetDutyCycle
{
    pub fn new(spi: T, dc_pin: U, rst_pin: V, bl_pin: W) -> Self {
        Self {
            spi, dc_pin, rst_pin, bl_pin, orientation: LcdOrientation::Rotate0,
        }
    }
    ...
    pub fn set_backlight(&mut self, value: u16) -> Result<(), LcdError> {
        self.bl_pin.set_duty_cycle(value).map_err(|_| LcdError::PinError)?;
        Ok(())
    }
    ...
    pub fn fill_rect(&mut self, x: u16, y: u16, w: u16, h: u16, color: u16) -> Result<(), LcdError> {
    ...
    pub fn draw_sprite(&mut self, x: u16, y: u16, w: u16, h: u16, data: &[u8]) -> Result<(), LcdError> {
        self.set_window(x, y, x + w , y + h )?;
